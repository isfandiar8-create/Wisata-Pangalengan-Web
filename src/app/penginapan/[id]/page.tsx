...
export default async function DetailPenginapan({ params }: Props) {
  ...
  // Mengubah "Rp 3.000.000" menjadi "3000000"
  const cleanPrice = (priceString: string) => {
    return priceString.replace(/[^0-9]/g, '');
  };

  // ─── FUNGSI PEMBUAT ULASAN DINAMIS UNTUK PENGINAPAN ───
  const getLodgingReviewData = (kategori: string, namaPenginapan: string) => {
    let data = { rating: "4.8", count: "85", reviewer: "Tamu Setia", text: `Pengalaman menginap yang sangat menyenangkan di ${namaPenginapan}. Fasilitas lengkap dan pelayanan sangat membantu.` };
    
    if (kategori.includes("Premium") || kategori.includes("Super") || kategori.includes("VIP")) {
      data = { rating: "5.0", count: "142", reviewer: "Tamu Eksklusif", text: `Luar biasa! ${namaPenginapan} benar-benar memberikan standar layanan VIP. Villa sangat bersih, mewah, dan sesuai ekspektasi rombongan kami.` };
    } else if (kategori.includes("Glamping") || kategori.includes("Cottage")) {
      data = { rating: "4.9", count: "118", reviewer: "Pasangan Muda", text: `Suasana di ${namaPenginapan} sangat asri dan estetik. Nyaman banget untuk pelarian dari kota, fasilitas air panasnya juga sangat membantu saat malam dingin.` };
    } else if (kategori.includes("Barak") || item.kapasitas.includes("30") || item.kapasitas.includes("45")) {
      data = { rating: "4.9", count: "205", reviewer: "Panitia Rombongan", text: `Solusi terbaik untuk rombongan besar! ${namaPenginapan} sangat luas, parkiran aman, dan fasilitas komunalnya sangat memudahkan panitia mengatur peserta.` };
    }
    
    return data;
  };

  const reviewData = getLodgingReviewData(item.kategori, item.nama);

  // ─── KODE SCHEMA MARKUP (JSON-LD) UNTUK PENGINAPAN ───
  // Menggunakan tipe LodgingBusiness yang valid digabungkan dengan Product (Agar Review Muncul)
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": ["LodgingBusiness", "Product"],
    "name": item.nama,
    "image": [
      `https://www.gopangalengan.id${item.image}`
    ],
    ...
    "offers": {
      ...
      "priceCurrency": "IDR",
      "price": cleanPrice(item.hargaWeekday),
